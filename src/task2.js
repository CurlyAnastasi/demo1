...
export default function analyseEnvelopes (env1, env2) {
    ...
    if (typeof env1 !== 'object' || Array.isArray(env1) || typeof env2 !== 'object' || Array.isArray(env2)) {
        return '{status: failed, reason: Unvalid type of params. Should be two objects}';
    };
    ...
    if (!env1.hasOwnProperty('a') || !env1.hasOwnProperty('b') || !env2.hasOwnProperty('c') || !env2.hasOwnProperty('d')) {
        return '{status: failed, reason: There is no such key in object as a, b, c or d}';
    };

    // Check that values of keys are numbers in range of 1 and 1000000
    for (let i in env1) {
        if (typeof env1[i] !== 'number' || env1[i] < minValue || env1[i] > maxValue) {
            return `{status: failed, reason: Unvalid values. Must be numbers from ${minValue} to ${maxValue}}`;
        };
    };
